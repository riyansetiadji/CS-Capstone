...
#ifndef PCL_IMPL_SKIN_FILTER_H_
#define PCL_IMPL_SKIN_FILTER_H_

#include <iostream>
#include <pcl/apps/hand_obj_tracker/skinFilter.h>

template <typename PointT> bool 
pcl::skinFilter<PointT>::addSamples (const PointCloudConstPtr &cloud_in)
{
	bool isSuccess = true;
	if (sampleFull)
	{
...
pcl::skinFilter<PointT>::identifySkin (PointCloud &cloud_skin, PointCloud &cloud_skin2, PointCloud &cloud_other)
{
	...
	if(sampleFull && isOrganziedInput())
	{
		...
		for (size_t i = 0; i < 320; ++i)
			for (size_t j = 0; j < 240; ++j)
			{
				...
				/*else if ((labels(i,j) == maxLabel2) && (maxLabelSize2>100))
				{
					cloud_skin2.points.push_back(input_->at(i,j));
					current_cloud->points.push_back(input_->at(i,j));
				}*/
...
#define PCL_INSTANTIATE_skinFilter(T) template class PCL_EXPORTS pcl::skinFilter<T>;


#endif  //#ifndef PCL_IMPL_SKIN_FILTER_H_
