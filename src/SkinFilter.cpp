...
void SkinFilter::identifySkin (PointCloud<PointXYZRGBA>::Ptr &cloud_skin,
			       PointCloud<PointXYZRGBA>::Ptr &cloud_other)
{
  if(sampleFull && ((cloud_other->height == 240) && (cloud_other->width == 320)))
	...
	for (size_t j = 0; j < 240; ++j)
	  {
	    rgb2uv(cloud_other->at(i,j).r, 
		   cloud_other->at(i,j).g, 
		   cloud_other->at(i,j).b, U, V);
	...
	for (size_t j = 0; j < 240; ++j)
	  {
	    ...
	    if ((labels(i,j) >0) && (labelSize[labels(i,j)]>100)) // output all blobs that have sufficient sizes
	      {
		cloud_skin->points.push_back(cloud_other->at(i,j));
		current_cloud->points.push_back(cloud_other->at(i,j));
	      }
	    /*else if ( !( (cloud_other->at(i,j).r == 0) && (cloud_other->at(i,j).g == 0) && (cloud_other->at(i,j).b == 0)) )
	      cloud_other->points.push_back(cloud_other->at(i,j));*/
	  }
