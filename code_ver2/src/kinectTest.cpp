  ...
  public:
    ...
    KinectTracking(int argc, char** argv)
		...
		segment_table_plane(const RefCloudConstPtr & cloud_in)
		{
			...
			RefPointType colorSeed = cloud_in->at(CLOUD_WIDTH/2,CLOUD_HEIGHT/2);
			table_cloud.reset(new RefCloud);
			for (size_t i = 0; i < cloud_in->points.size (); i++)
			{
				bool c_filter = (abs(colorSeed.r-cloud_in->points[i].r)<60) && 
												(abs(colorSeed.g-cloud_in->points[i].g)<60) &&
												(abs(colorSeed.b-cloud_in->points[i].b)<60);
				if (c_filter)
					table_cloud->points.push_back (cloud_in->points[i]);
			}
		...
		cloud_cb (const RefCloudConstPtr & cloud_in)
		{
			...
			for (size_t i = 0; i < cloud_in->points.size (); ++i)
			{
				if (( cloud_in->points[i].z < z_limit_) && ( cloud_in->points[i].x < 0.5))
				{
			...
			if (object_status == OBJ_TRACKING)
			{
				obj_Tracker.setInputCloud(filtered_cloud);
			...
			if (event.keyUp())
			{
				switch (event.getKeyCode())
				{
					case 109: //"m"
						...
						if (object_status == OBJ_TRACKING)
						{
							pcl::console::print_highlight ("stop object Tracking ... \n");
							object_status = OBJ_UNKNOWN;
						}
						break;
					case 105: //"i"
					...
					case 98: //"b"
						displayBG = !displayBG;
						main_viz->addCoordinateSystem(0.1,table_trans);
						break;
					case 116: //"t"
						if (hand_status != HAND_IDENTIFYING)
						{
		...
		run ()
		{
			...
			// Render and process events in the two interactors
				...
				if (filtered_cloud && cld_mutex.try_lock ())
				{
					if (!viewer_init)
					{
						main_viz->getRenderWindow ()->SetSize (1280, 720);
					...
					//if (!main_viz->updatePointCloud (table_cloud,"tCloud"))
					//{
					//	main_viz->addPointCloud (table_cloud, "tCloud");
					//}
